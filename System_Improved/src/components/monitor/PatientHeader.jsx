...
const PatientHeader = ({ caseData, detailedData }) => {
    ...
    if (!caseData?.isQueryCase) {
        if (caseData?.diagnosis && caseData.diagnosis.length > 0) {
            symptom = caseData.diagnosis[0]; // e.g. "nonspecific st..."
        ...
        <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 shrink-0 flex justify-between items-center">
            <div>
                <h2 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                    {id}
                    <span className="text-xs font-normal text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{gender}, {age}y</span>
                </h2>
                <div className="text-xs text-slate-500 mt-1 flex gap-3">
                    <span className="text-red-500 font-medium">⚠️ {symptom}</span>
                    <span>• {time}</span>
                </div>
            </div>
